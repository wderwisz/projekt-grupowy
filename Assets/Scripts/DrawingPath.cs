...
public class DrawingPath : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        {
            Debug.Log("PathManager nie jest przypisany w DrawingPath. Nie można ustawić drawingPathInstance w PathManager.");
        }
    }
                ...
                if (rayInteractor.TryGetCurrent3DRaycastHit(out RaycastHit hit))
                {
                    ...
                    else //nie trafienie w kropke
                    {
                        if (coloringStarted)
                        {
                            ...
                            if (consecutiveMisses >= 25) // manipulacja dokadnością 25 nietrafien dopiero zmniejsza dokadność
                            {
    ...
    public void SavePath(string nameToSave)
    {
        ...
        {
            Debug.LogError("Nie można zapisac z pustą ścieżką.");
            return;
        }
    ...
    public void ResetDrawingState()
    {
        ...
        Debug.Log("DrawingPath: Stan rysowania (lastDotPosition) został zresetowany i rysowanie będzie zablokowane na jedną klatkę.");
    }
}
